...
if [[ $flag_split == true ]]; then
	...
	if [[ $flag_cleaned == false ]]; then 
		if [[ $flag_splitted == true ]]; then
			bash ../lib/limpia_corpus.sh "$punct2func" -s "$split_LINES" -wto "$ruta/corpus/split_${salida}_out/$salida" $entrada;
		else
			bash ../lib/limpia_corpus.sh "$punct2func" -s "$split_LINES" -wo "$ruta/corpus/split_${salida}_out/$salida" $entrada;
		fi
	fi
	bash ../lib/vocabulario_freqs.sh -ro "$ruta/out/${salida}" "$ruta/corpus/split_${salida}_out"/*
else
	...
	bash ../lib/vocabulario_freqs.sh -ro "$ruta/out/${salida}" "$ruta/corpus/${salida}_out"
fi
